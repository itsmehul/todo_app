...
pub async fn find_task_by_id(
    ...
    task.ok_or_else(|| {
        ...
        AppError::new("not found", StatusCode::NOT_FOUND)
    })
}
...
pub async fn save_active_task(
    ...
    let task = task.save(db).await.map_err(|error| {
        ...
        AppError::new("Error saving task", StatusCode::INTERNAL_SERVER_ERROR)
    })?;
...
pub async fn get_all_tasks(
    ...
    query.all(db).await.map_err(|error| {
        ...
        AppError::new("Error getting all tasks", StatusCode::INTERNAL_SERVER_ERROR)
    })
}
